...
behavData.animal<-behavData.all[behavData.all$cond == "LAnPr" | behavData.all$cond == "LAnTar" | behavData.all$cond == "HAnPr" | behavData.all$cond == "HP_AnTar", ]
...
##compute overall SD
LPHPprimetarg<-behavData.all[behavData.all$cond == "LAnPr" | behavData.all$cond == "LAnTar" | behavData.all$cond == "HAnPr" | behavData.all$cond == "HP_AnTar", ]
...
library('ez')
eztest <-ezANOVA(data=behavData.animal,dv = .(acc),wid=.(subj),within=.(prop, pos),type=2,detailed=TRUE)
print(eztest)
